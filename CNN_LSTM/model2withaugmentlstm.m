%% ============================================================
% MODEL 2: AUGMENTED CNN + PROPER LSTM (FIXED)
% Fixed: Properly handles categorical labels in augmentation
%% ============================================================
...
numTimeWindows = 10;     % Changed from 4 → 10
windowOverlap = 0.5;     % Changed from 0.0 → 0.5
lstmEpochs = 50;
lstmBatchSize = 16;
...
fprintf('  Original classes: %d\n', numClasses);
fprintf('  Augmented classes: %d\n', numel(uniqueLabelsAug));
if numel(uniqueLabelsAug) ~= numClasses
    error('Label mismatch! Augmented data has %d classes but expected %d', ...
        numel(uniqueLabelsAug), numClasses);
end
fprintf('  ✓ Labels verified!\n\n');
...
function [augImages, augLabels] = generateAugmentedData(images, labels, factor)
    % FIXED: Properly handles categorical labels
    [H, W, C, N] = size(images);
    totalAugmented = N * factor;
    
    augImages = zeros([H, W, C, totalAugmented], 'single');
    % Create cell array to collect labels, then convert to categorical at end
    ...
    for i = 1:N
        ...
        % Store original
        augImages(:,:,:,idx) = img;
        augLabelsCell{idx} = char(label);  % Convert to string
        idx = idx + 1;
        ...
        for j = 1:(factor-1)
            ...
            augLabelsCell{idx} = char(label);  % Same label as original
            idx = idx + 1;
        end
    end
    
    % Convert cell array of strings to categorical
...
function augImg = applyTimeStretch(img, stretchFactor)
    ...
    if newWidth > W
        ...
        % Manual padding instead of padarray
        padSize = W - newWidth;
